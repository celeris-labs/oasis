...
class HTTPReadConfig : public libstf::Config {
  public:
    ...
    ///   [3:0]   HTTPRead FSM   (0=IDLE,1=TCP_INIT,2=TCP_SEND,3=TCP_READ,4=CLOSE)
    ///   [7:4]   tcp_init FSM   (0=IDLE,1=SEND,2=WAIT/DONE)
    ///   [11:8]  tcp_send FSM   (0=IDLE,3=BUILD,4=META,5=DATA,6=WAIT/DONE)
    ///   [15:12] tcp_read FSM   (0=IDLE,7=WAIT_NOTIFY,8=REQ_PKG,9=RECV/DONE)
    ///   [16] init_done [17] init_error [18] send_done [19] send_error
    ///   [20] read_done [21] read_error [22] handler_busy
    /// Returns 0 on bitstreams built before this CSR existed.
    uint32_t debug_status();

    /// Read back a per-stream write CSR (server_ip=0, server_port=1, file_len=2, range_begin=11,
    /// range_end=12, size=13, start=14).
