...
void oasis::CallbackOperator::apply(libstf::stream_t stream, oasis::OasisContext &ctx) {
    callback_(stream, ctx);
}

void oasis::CallbackOperator::print(std::ostream &os) const {
    os << "CallbackOperator(" << name_ << ")";
}

void oasis::StreamConfigOperator::apply(libstf::stream_t stream, oasis::OasisContext &ctx) {
    // The Bloom/bypass mux is physically on stream 0. For other streams this is a no-op.
    if (stream != 0) {
        return;
    }

    auto config = ctx.config<libstf::StreamConfig>();
    config->enqueue_stream_config(0, type_, select_);
}

void oasis::StreamConfigOperator::print(std::ostream &os) const {
    os << "StreamConfigOperator(type=" << static_cast<int>(type_)
       << ", select=" << static_cast<int>(select_) << ")";
}
