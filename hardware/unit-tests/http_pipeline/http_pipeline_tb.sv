...
module http_pipeline_tb #(
    ...
    initial begin
        ...
        forever begin
            ...
            if (txmeta_valid && txmeta_ready) begin
                ...
                // Which request is this? The bench encodes r in the third Range-begin digit.
                r = -1;
                for (p = 0; p + 6 < get_txt.len(); p++) begin
                    if (get_txt.substr(p, p+5) == "bytes=") begin
                        r = int'(get_txt.getc(p+8)) - 8'h30;
                        p = get_txt.len();
                    end
                end
                if (r < 0 || r >= NUM_REQS) $fatal(1, "could not identify request in GET: %s", get_txt);
    ...
    function automatic http_config_t make_req(input int r);
        ...
        begin
            ...
            // Distinct ranges per request. The third begin digit encodes r, which is how the tx
            // model tells which request a GET is -- including a replayed one.
...
`endif
    ...
    initial begin
        ...
        if (opens_seen != 2) begin
            $error("expected 2 connections (initial + one reconnect), saw %0d", opens_seen);
            errors++;
        end
        if (fins_sent != 1) begin
            $error("the server was supposed to FIN exactly once, it sent %0d", fins_sent);
            errors++;
        end
        // Replay: every request that was sent but not answered when the connection died must go out
        // again. How many that is depends on the ring depth -- with NUM_SLOTS slots at most that
        // many requests can be outstanding -- so the bound is a range, not a number. What must hold
        // either way is that at least one GET was replayed and none was replayed spuriously.
        begin
            automatic int max_replay = (NUM_SLOTS < (NUM_REQS - RECONNECT_AFTER - 1)) ?
                                        NUM_SLOTS : (NUM_REQS - RECONNECT_AFTER - 1);
            if (gets_seen <= NUM_REQS) begin
                $error("no GET was replayed after the reconnect: saw %0d for %0d requests",
                       gets_seen, NUM_REQS);
                errors++;
            end else if (gets_seen > NUM_REQS + max_replay) begin
                $error("replayed more GETs than could have been outstanding: saw %0d, at most %0d expected",
                       gets_seen, NUM_REQS + max_replay);
                errors++;
            end
        end
        ...
        // sequential and correctness (checked above) is the whole contract. Only request 1 is
        // checked: the reconnect deliberately serialises the tail of the run.
