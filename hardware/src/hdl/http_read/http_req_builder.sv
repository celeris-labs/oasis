module http_req_builder (
    ...
    always_comb begin
        file_words[0] = fileWord0; file_words[1] = fileWord1;
    ...
    // ---------------------------------------------------------------
    // Combinational Logic (FSM + write-bus)
    // ---------------------------------------------------------------
    always_comb begin
        state_d     = state_q;
        idx_d       = idx_q;
        ...
        case (state_q)
            ...
            IP: begin
                write_byte = ipHex[sub_idx_q*8 +: 8];
                write_en   = 1'b1;
                idx_d      = idx_q + 1'b1;
                sub_idx_d  = sub_idx_q + 1'b1;

                if (sub_idx_q == (ipHexLen - 1) || ipHexLen == 0) begin
                    state_d   = COLON;
                    sub_idx_d = '0;
                end
            end
            ...
            PORT: begin
                write_byte = portHex[sub_idx_q*8 +: 8];
                write_en   = 1'b1;
                idx_d      = idx_q + 1'b1;
                sub_idx_d  = sub_idx_q + 1'b1;

                if (sub_idx_q == (LEN_PORT - 1)) begin
                    state_d   = HOST_CRLF;
                    sub_idx_d = '0;
                end
            end
            ...
            RANGE_PRE: begin
                ...
                if (sub_idx_q == (LEN_RANGE_PRE - 1)) begin
                    state_d   = (rangeBeginLen == 0) ? RANGE_DASH : RANGE_BEGIN;
                    sub_idx_d = '0;
                end
            end

            RANGE_BEGIN: begin
                write_byte = range_begin_words[sub_idx_q[4:2]][sub_idx_q[1:0]*8 +: 8];
                write_en   = 1'b1;
                idx_d      = idx_q + 1'b1;
                sub_idx_d  = sub_idx_q + 1'b1;

                if (sub_idx_q == (rangeBeginLen - 1)) begin
                    state_d   = RANGE_DASH;
                    sub_idx_d = '0;
                end
            end
            ...
            RANGE_DASH: begin
                ...
                write_en   = 1'b1;
                idx_d      = idx_q + 1'b1;
                state_d    = (rangeEndLen == 0) ? RANGE_CRLF : RANGE_END;
                sub_idx_d  = '0;
            end

            RANGE_END: begin
                write_byte = range_end_words[sub_idx_q[4:2]][sub_idx_q[1:0]*8 +: 8];
                write_en   = 1'b1;
                idx_d      = idx_q + 1'b1;
                sub_idx_d  = sub_idx_q + 1'b1;

                if (sub_idx_q == (rangeEndLen - 1)) begin
                    state_d   = RANGE_CRLF;
                    sub_idx_d = '0;
                end
            end
    ...
    // ---------------------------------------------------------------
    // Sequential Logic (Pure FF mapping + decoded byte write)
    // ---------------------------------------------------------------
