...
> **Why purge before rebuilding, and why the `CMAKE_DISABLE_FIND_PACKAGE_*` flags?**
> `software/CMakeLists.txt` (and `parcore/software/CMakeLists.txt`) only build
> `Coyote`/`libstf`/`parcore` from the submodule source the *first* time (via `add_subdirectory`);
> on every later configure they prefer `find_package(...)`, which silently reuses whatever was
> already installed at `$HOME/opt` — even if the submodule pins in this checkout have since moved
> (e.g. after `git checkout` to a different branch, or `git submodule update`). The
> `CMAKE_DISABLE_FIND_PACKAGE_*` flags stop that reuse and force a rebuild from the currently
> checked-out submodule source.
>
> That alone isn't sufficient, though: `parcore/libstf/software/CMakeLists.txt` adds
> `JEMALLOC_INCLUDE_DIRS` (`$HOME/opt/include`) as a `PUBLIC` include dir on the `libstf` target,
> so that whole directory leaks into the include path of every target that links `libstf`
> (`parcore`, `oasis`, ...). If a previous install left old headers there (e.g.
> `$HOME/opt/include/parcore/configuration.hpp`), the compiler can pick up that stale header
> instead of the fresh one from the submodule, even though `libparcore.so` itself got rebuilt
> correctly — producing a mismatch like `ColumnChunkDecoderConfig::enqueue_column_chunk`'s
> `has_levels` parameter appearing or disappearing depending on what was last installed. Removing
> the old install directories first guarantees there is nothing left to shadow.
>
> **Why `LD_LIBRARY_PATH` before `make` in `extension/`, not just before running the binary?**
> None of `liboasis.so`/`liblibstf.so`/`libparcore.so` are built with an `RPATH`/`RUNPATH`
> (`readelf -d` shows none), so when the extension's build links executables against them, `ld`
> has no way to locate their own transitive dependencies (`libcoyote.so`, `libjemalloc.so.2`)
> unless it's told where to look. `ld` falls back to searching `LD_LIBRARY_PATH` for such
> transitively-needed shared objects when no `-rpath-link`/`RUNPATH` is available, so it must be
> exported before invoking `make` here — the same variable Step 4.2 exports later for the
> *runtime* loader is also needed at *link* time.
