...
// Coyote's <syslog.h> (pulled in transitively via oasis_context.hpp, only in EN_SIMULATION builds
// where the simulation cThread.hpp includes it) defines LOG_DEBUG / LOG_INFO / LOG_WARNING as
// numeric macros that collide with the duckdb::LogLevel enum values used in logger.hpp, turning
// e.g. `LogLevel::LOG_WARNING` into `LogLevel::4`. #undef them before including the logger so the
// LogLevel:: use sites below (and in TUs that include this header, e.g. oasis_scan.cpp) compile.
